...
| 我要撰寫...     | 選擇模板                          | 預估時間   |
| --------------- | --------------------------------- | ---------- |
| Sprint 規劃文檔 | `sprint-template.md`              | 10-15 分鐘 |
| 技術決策記錄    | `adr-template.md`                 | 15-20 分鐘 |
| 快速開始指南    | `guide-template-basic.md`         | 5-10 分鐘  |
| 完整技術指南    | `guide-template-standard.md`      | 15-20 分鐘 |
| 深度架構指南    | `guide-template-comprehensive.md` | 30+ 分鐘   |

💡 **不確定選哪個？** Guide 建議從 Standard 開始。
...
```bash
# 範例：創建一個 Guide 文檔
cp docs/templates/guide-template-standard.md docs/guides/setup/my-guide.md
```
...
打開檔案，找到 frontmatter (檔案頂部 `---` 之間的部分)，填寫標記為🔴的必填欄位：

```yaml
---
# 🔴 必填欄位範例
title: 'Supabase 本地開發設定'
type: 'guide'
category: 'setup'
difficulty: 'beginner'
estimated_time: '15-20 分鐘'
tags: ['supabase', 'local', 'development']
...
# 或手動檢查
pnpm lint:md docs/guides/setup/my-guide.md
```
...
你已經創建了第一份標準化文檔。需要更詳細的說明？繼續閱讀下面的完整指南。
...
內文中使用 `[欄位名]` 來引用 frontmatter 資料，確保資訊一致性。

**重要**：模板中的 `[欄位名]` 是**佔位符**，你需要**手動替換**為實際值。
...
> **持續時間**: [duration] | **開始**: [start_date]

<!-- 手動替換為 -->

> **持續時間**: 10-15 小時 | **開始**: 2025-11-25
```

**2. Emoji 欄位變數** - 需要查表轉換

帶 `_emoji` 後綴的欄位需要查詢 [Status Emoji 指南](./status-emoji-guide.md) 轉換：
...
**完整對照表**：參閱 [Status Emoji 指南](./status-emoji-guide.md)
...
2. **逐步替換檢查清單**:
   - [ ] 替換所有 `[欄位名]` 一般變數
   - [ ] 查表轉換所有 `[欄位名_emoji]` 變數
   - [ ] 搜尋檔案確認無剩餘 `[` 符號（排除程式碼區塊）

3. **常見錯誤**:
   - ❌ 直接複製 `[status_emoji]` 到文檔中
   - ❌ 忘記查表，亂用 emoji（如用 ⚙️ 表示進行中）
   - ✅ 總是查詢 Status Emoji 指南確保一致性
